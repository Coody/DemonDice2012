...
@implementation FirstPlayTeachLayer

-(void)pressedRockDiceButton{
    if( lockRedButton == YES ){
...
-(void)createDiceWithPosition:(int)gameboardPosition withDiceType:(EnumDiceNumberAndColor)tempDiceType{
	Dice *tempCreateDice = [[Dice alloc]initWithFile:[self getDicePicture:tempDiceType] withDiceType:tempDiceType withRecentPosition:ccp(100 + 60 * (gameboardPosition%4) , 40 + 60 * (gameboardPosition/4))];
	[diceArray addObject:tempCreateDice];
	[tempCreateDice setPosition:ccp(100 + 60 * (gameboardPosition%4) , 40 + 60 * (gameboardPosition/4))];
	[self addChild:tempCreateDice z:enumStableZOrder];
	[tempCreateDice setGameBoardPosition:gameboardPosition];
    [self addDiceToHashGameBoard:tempCreateDice];
    isGameBoardHasDice[tempCreateDice.gameBoardPosition] = YES;
    [tempCreateDice setCanMoveOrNot:NO];
...
-(void)createDice{
	...
	if ( [diceArray count] >= (MaxDiceNumber - 4) && [superDiceArray count] == 0 ) {
        ...
        lockPlayDiceLayer = YES;
        lockRedButton = YES;
        [self changeTalkContext:nil withNumber:countText];
        if ( firstPlayNumber == enumFirstPlayFourDiceComboCondition || firstPlayNumber == enumFirstPlaySuperDiceCreateCondition) {
            lockChangeTalkBubble = YES;
            return;
        }
		for (int i = 0; i< createDiceNumber; i++) {
            int randomDiceNumber ;
            switch ( firstPlayNumber ) {
                case enumFirstPlayPressButtonCondition:
                    randomDiceNumber = enumDice01Red;
                    break;
                case enumFirstPlayFourDiceCondition:
                    randomDiceNumber = enumDice01Yellow;
                    break;
                case enumFirstPlayFourDiceCondition2:
                    randomDiceNumber = enumDice01Blue;
                    break;
                case enumFirstPlayFourDiceCondition3:
                    randomDiceNumber = enumDice01Green;
                    break;
                case enumFirstPlayFourDiceCondition4:
                {
                    if ( i == 0 ) {
                        ...
                        randomDiceNumber = enumDice02Red;
                    }
                    else if( i ==2 ){
                        randomDiceNumber = enumDice03Red;
                    }
                    else {
                        randomDiceNumber = enumDice04Red;
                    }
                }
                    break;
                ...
                case enumFirstPlayThreeDifRuleCondition2:
                {
                    if ( i == 0 ) {
                        ...
                        randomDiceNumber = enumDice02Green;
                    }
                    else if( i ==2 ){
                        randomDiceNumber = enumDice03Red;
                    }
                    else{
                        randomDiceNumber = enumDice04Yellow;
                    }
                }
                    break;
                case enumFirstPlayThreeDifRuleCondition3:
                    ...
                    break;
                default:
                    randomDiceNumber = ((arc4random()%16) + 1);
                    break;
            }
...
//                        randomDiceNumber = 9;
			...
			Dice *tempCreateDice = [[Dice alloc]initWithFile:[self getDicePicture:randomDiceNumber] withDiceType:randomDiceNumber withRecentPosition:ccp( 40 , 40+(60*i))];
			[createDiceArray addObject:tempCreateDice];
			[tempCreateDice setPosition:ccp( 40 , 40+(60*i))];
...
-(void)runCountScoreAnimation{
	...
	CCLOG(@"countScoreArray = %d" , [countScoreArray count]);
...
-(id)init{
	...
	if (self != nil) {
		...
		CCMenuItemImage *rollButtonMenuItem = [CCMenuItemImage itemFromNormalImage:@"rollButton.png" selectedImage:@"rollButtonPressed.png" target:self selector:@selector(pressedRockDiceButton)];
		...
		rollButtonMenu = [CCMenu menuWithItems:rollButtonMenuItem, nil];
        ...
        //[self schedule:@selector(changeTalkContext:) interval:10.0f];
        
		/* first,check data exist or not , if not , create it. if existed , load it . */
        CCMenuItemImage *backButtonItem = [CCMenuItemImage itemFromNormalImage:@"backSingleGameButton.png"
																 selectedImage:@"backSingleGameButtonSelected.png"
																		target:self
																	  selector:@selector(runSingleGameScene)];
        backToSingleGameButton = [CCMenu menuWithItems:backButtonItem, nil];
		[self addChild:backToSingleGameButton z:200];
		[backToSingleGameButton setPosition:ccp(screenSize.width*0.038, screenSize.height*0.975)];
		[backToSingleGameButton setOpacity:0];
		id showBackButton = [CCSequence actions:delayShowBubble,delayShowBubble,[CCFadeIn actionWithDuration:0.3], nil];
		[backToSingleGameButton runAction:showBackButton];
        
        skill03CkeckScore = 0;
        
        firstPlayNumber = enumFirstPlayStart;
        [talkLabel setChapter:firstPlayNumber];
        countText = 1;
        lockRedButton = YES ;
        lockChangeContext = YES;
        lockChangeTalkBubble = YES;
        lockDiceBufferArea = YES ;
        lockPlayDiceLayer = YES ;
        
        [rollButtonMenu setIsTouchEnabled:NO];
        
        [[GameManager sharedGameManager] playBackgroundTrack:FIRST_PLAY];
        
        
        
        /*這裡記得要打開！現在測試用所以不打開！(不會有存檔所以不會進入單機玩狀態！)*/
		/*first,check data exist or not , if not , create it. if existed , load it . */
...
-(void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
	tempMovingDice = nil;
    if ( lockChangeTalkBubble == YES ) {
        switch ( firstPlayNumber ) {
            case enumFirstPlayStart:{
                switch ( countText ) {
                    case 4:
                        firstPlayNumber = enumFirstPlayPressButtonCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                        break;
                }
                break;
            ...
            case enumFirstPlayPressButtonCondition:
                switch ( countText) {
                    case 6:
                        lockRedButton = NO;
                        lockPlayDiceLayer = NO;
                        [rollButtonMenu setIsTouchEnabled:YES];
                        [talkLabel setEventTextWithSequence:countText];
                        break;
                    case 7:
                        if ( [createDiceArray count] == 4 ) {
                            CCLOG(@"有成功按下喔！！！");
                            lockPlayDiceLayer = YES;
                            lockRedButton = YES;
                            [rollButtonMenu setIsTouchEnabled:NO];
                            ...
                            CCLOG(@"沒成功！？？");
                            [talkLabel setEventTextWithSequence:countText-1];
                            countText--;
                        }
                        break;
                    case 8:
                        firstPlayNumber = enumFirstPlayGameBoardCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
                        CCLOG(@"countText = %d" , countText);
                        [talkLabel setEventTextWithSequence:countText];
                        //lockRedButton = YES;
                        countText++;
                        break;
            ...
            case enumFirstPlayGameBoardCondition:
                switch ( countText ) {
                    case 7:{
                        tempShowSprite = [CCSprite spriteWithFile:@"4x4.png"];
                        [tempShowSprite setPosition:ccp(186,130)];
                        [self addChild:tempShowSprite z:1000];
                        id fadeInAction = [CCFadeIn actionWithDuration:0.1f];
                        [tempShowSprite runAction:fadeInAction];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    }
                        break;
                    case 8:{
                        [tempShowSprite removeFromParentAndCleanup:YES];
                        tempShowSprite = [CCSprite spriteWithFile:@"4x1.png"];
                        [self addChild:tempShowSprite z:1000];
                        [tempShowSprite setPosition:ccp(192, 281)];
                        id fadeInAction = [CCFadeIn actionWithDuration:0.1f];
                        [tempShowSprite runAction:fadeInAction];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 12:
                        [tempShowSprite removeFromParentAndCleanup:YES];
                        firstPlayNumber = enumFirstPlayFourDiceCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayFourDiceCondition:
                switch ( countText ) {
                    case 1:
                        ...
                        break;
                    case 4:
                        tempShowSprite = [CCSprite spriteWithFile:@"gameBoardPosition1.png"];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [self addChild:tempShowSprite z:1000];
                        id fadeInAction = [CCFadeIn actionWithDuration:0.1f];
                        [tempShowSprite runAction:fadeInAction];
                        [talkLabel setEventTextWithSequence:countText];
                    ...
                    case 5:
                        [tempShowSprite removeFromParentAndCleanup:YES];
                        tempShowSprite = [CCSprite spriteWithFile:@"gameBoardPosition2.png"];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [self addChild:tempShowSprite z:1000];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 6:
                        [tempShowSprite removeFromParentAndCleanup:YES];
                        tempShowSprite = [CCSprite spriteWithFile:@"gameBoardPosition3.png"];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [self addChild:tempShowSprite z:1000];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 7:
                        [tempShowSprite removeFromParentAndCleanup:YES];
                        tempShowSprite = [CCSprite spriteWithFile:@"gameBoardPosition4.png"];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [self addChild:tempShowSprite z:1000];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 11:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayFourDiceCondition2;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayFourDiceCondition2:
                switch ( countText ) {
                    ...
                    case 6:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayFourDiceCondition3;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayFourDiceCondition3:
                switch ( countText ) {
                    ...
                    case 4:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayFourDiceCondition4;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayFourDiceCondition4:
                switch ( countText ) {
                    ...
                    case 7:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayThreeDifRuleCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayThreeDifRuleCondition:
                switch ( countText ) {
                    case 4:
                        tempShowSprite = [CCSprite spriteWithFile:@"threeDifRule1.png"];
                        [self addChild:tempShowSprite z:1000];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                        tempShowSprite = [CCSprite spriteWithFile:@"threeDifRule2.png"];
                        [self addChild:tempShowSprite z:1000];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 7:
                        ...
                        break;
                    case 8:
                        tempShowSprite = [CCSprite spriteWithFile:@"threeDifRule3.png"];
                        [self addChild:tempShowSprite z:1000];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 9:
                        ...
                        break;
                    case 10:
                        tempShowSprite = [CCSprite spriteWithFile:@"threeDifRule4.png"];
                        [self addChild:tempShowSprite z:1000];
                        [tempShowSprite setPosition:ccp(160, 172)];
                        [talkLabel setEventTextWithSequence:countText];
                        countText++;
                    ...
                    case 15:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayThreeDifRuleCondition2;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayThreeDifRuleCondition2:
                switch ( countText ) {
                    ...
                    case 5:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayThreeDifRuleCondition3;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayThreeDifRuleCondition3:
                switch ( countText ) {
                    ...
                    case 10:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayFourDiceComboCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayFourDiceComboCondition:
                switch ( countText ) {
                    case 3:
                        ...
                        lockPlayDiceLayer = NO;
                        lockRedButton = NO;
                        [self createDiceWithPosition:0 withDiceType:enumDice03Red];
                        [self createDiceWithPosition:2 withDiceType:enumDice02Blue];
                        [self createDiceWithPosition:5 withDiceType:enumDice02Red];
                        [self createDiceWithPosition:6 withDiceType:enumDice03Yellow];
                        [self createDiceWithPosition:8 withDiceType:enumDice01Green];
                        [self createDiceWithPosition:9 withDiceType:enumDice01Yellow];
                        [self createDiceWithPosition:11 withDiceType:enumDice01Blue];
                        [self createDiceWithPosition:14 withDiceType:enumDice04Green];
                        [self createDiceWithPosition:15 withDiceType:enumDice04Red];
                        if ( [createDiceArray count] != 1 ) {
                            firstPlayNumber = enumFirstPlayThreeDifRuleCondition3;
                            [self createDice];
                            firstPlayNumber = enumFirstPlayFourDiceComboCondition;
                        }
                    ...
                    case 7:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlaySuperDiceCreateCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
                ...
                break;
                //TODO:SuperDiceCreateCondition
            case enumFirstPlaySuperDiceCreateCondition:
                switch ( countText ) {
                    case 2:
                        tempShowSprite = [CCSprite spriteWithFile:@"superDiceCombo.png"];
                        [tempShowSprite setPosition:ccp(144, 280)];
                        [self addChild:tempShowSprite z:1000];
                        [tempShowSprite setOpacity:0];
                        [talkLabel setEventTextWithSequence:countText];
                        lockPlayDiceLayer = NO;
                        lockRedButton = NO;
                        
                        [self createDiceWithPosition:1 withDiceType:enumDice03Yellow];
                        [self createDiceWithPosition:5 withDiceType:enumDice03Yellow];
                        [self createDiceWithPosition:13 withDiceType:enumDice03Yellow];
                        [self createDiceWithPosition:8 withDiceType:enumDice01Red];
                        [self createDiceWithPosition:10 withDiceType:enumDice01Red];
                        [self createDiceWithPosition:11 withDiceType:enumDice01Red];
                        [self createDiceWithPosition:9 withDiceType:enumDice04Green];
                        [self pressedRockDiceButton];
                        break;
                    case 3:
                        lockRedButton = YES;
                    ...
                    case 11:
                        lockPlayDiceLayer = YES;
                        lockChangeTalkBubble = NO;
                        if ( [superDiceArray count] < 1 ) {
                    ...
                    case 17:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayGameBoardBuffCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayGameBoardBuffCondition:
                switch ( countText ) {
                    ...
                    case 6:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayChangeTalkBubbleCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
            ...
            case enumFirstPlayChangeTalkBubbleCondition:
                switch ( countText ) {
                    case 20:
                        ...
                        lockRedButton = YES;
                        [rollButtonMenu setIsTouchEnabled:NO];
                        firstPlayNumber = enumFirstPlayEndCondition;
                        [talkLabel setChapter:firstPlayNumber];
                        countText = 1;
                        break;
                    default:
    ...
    CCLOG(@"context = %d",countText);
    if ( lockPlayDiceLayer ) {
        return;
    }
//    if ( firstPlayNumber == enumFirstPlaySuperDiceCreateCondition ) {
//        if ( [self checkDicePosition] ) {
//            
//        }
//    }
...
-(void)ccTouchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
	...
	if ( tempMovingDice.diceType != enumDiceGold ) {
        ...
        if ( lockDiceBufferArea ) {
            if ( touchLocation.y > 228 ) {
                tempY = 228;
            }
        ...
        [tempMovingDice setPosition:ccp( tempX, tempY )];
	}
	else{
        if ( touchLocation.x > 298) {
            tempX = 298;
        ...
        [tempMovingDice setPosition:ccp(( tempX - (superDiceBoard.position.x - superDiceBoard.contentSize.width*0.5)), (tempY - (superDiceBoard.position.y - superDiceBoard.contentSize.height*0.5)))];
	}
}
...
-(void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
	...
	if ( !lockChangeTalkBubble && tempMovingDice == NULL && touchLocation.y > 320 && touchLocation.x > 150 && touchLocation.y <= 420 && touchLocation.x <= 260){
    ...
    else if( !lockChangeContext && tempMovingDice == NULL && touchLocation.y > 320 && touchLocation.x < 150 && talkBubbleTag == enumTalkBubbleNPCTalk ){
	...
	for ( Dice *tempDice in buffDiceArray ) {
		...
		if( tempDice.zOrder == tempMovingDice.zOrder ){
			continue;
			/* if dice is itself and do not change anything. */
		}
		if ( CGRectContainsPoint(leavePosition, touchLocation) ) {
			CGPoint tempSaveReplacePosition = tempDice.recentPosition;
			int tempGameBoardPosition = tempDice.gameBoardPosition;
    ...
    int maxDiceNumber = MaxDiceNumber;
    if ( lockDiceBufferArea ) {
        maxDiceNumber = 16;
    }
	for (int i = 0 ; i < maxDiceNumber ; i++) {
		...
		/* tempMovingDice move to gameBoard(gameBoard does not have dice). */
			...
			if( CGRectContainsPoint( gameBoardPosition , touchLocation ) ){
				...
				tempMovingDice = nil;
                /* 這裡要注意的是，在begin那裡的countText就會++了，所以這裡只要條件達成就不會退回上一步，但是條件失敗就會退回上一步。 */
                if ( [self checkDicePosition] ) {
                    lockRedButton = NO;
                    [rollButtonMenu setIsTouchEnabled:YES];
                    [talkLabel setEventTextWithSequence:countText];
...
-(BOOL)checkDicePosition{
    BOOL result = NO;
    Dice *tempCheckPositionDiceArray[4];
    int i = 0;
    i = 0;
    for ( Dice *tempCheckPositionDice in createDiceArray) {
        if ( tempCheckPositionDice.gameBoardPosition == -1) {
            return result;
        }
        tempCheckPositionDiceArray[i] = tempCheckPositionDice;
        i++;
    }
    switch ( firstPlayNumber ) {
        case enumFirstPlayFourDiceCondition:
        {
        if (tempCheckPositionDiceArray[0].gameBoardPosition%4 == tempCheckPositionDiceArray[1].gameBoardPosition%4 &&
            tempCheckPositionDiceArray[0].gameBoardPosition%4 == tempCheckPositionDiceArray[2].gameBoardPosition%4 &&
            tempCheckPositionDiceArray[0].gameBoardPosition%4 == tempCheckPositionDiceArray[3].gameBoardPosition%4) {
            result = YES;
        }
        }
            break;
        ...
        for (int j = 3 ; j > 0; j--) {
            for ( int k = 1 ; k <= j ;  k++) {
                if (tempCheckPositionDiceArray[k-1].gameBoardPosition > tempCheckPositionDiceArray[k].gameBoardPosition) {
                    Dice *tempChangeDice = tempCheckPositionDiceArray[k-1];
                    tempCheckPositionDiceArray[k-1] = tempCheckPositionDiceArray[k];
                    tempCheckPositionDiceArray[k] = tempChangeDice;
                }
            }
        }
        CCLOG(@"dice position = ( %2d , %2d , %2d , %2d )" , tempCheckPositionDiceArray[0].gameBoardPosition , tempCheckPositionDiceArray[1].gameBoardPosition , tempCheckPositionDiceArray[2].gameBoardPosition , tempCheckPositionDiceArray[3].gameBoardPosition );
        if ( tempCheckPositionDiceArray[1].gameBoardPosition - tempCheckPositionDiceArray[0].gameBoardPosition == 1 && (tempCheckPositionDiceArray[0].gameBoardPosition != 3 || tempCheckPositionDiceArray[0].gameBoardPosition != 7 || tempCheckPositionDiceArray[0].gameBoardPosition != 11 || tempCheckPositionDiceArray[0].gameBoardPosition != 12)) {
            if ( tempCheckPositionDiceArray[0].gameBoardPosition + 4 == tempCheckPositionDiceArray[2].gameBoardPosition &&  tempCheckPositionDiceArray[1].gameBoardPosition + 4 == tempCheckPositionDiceArray[3].gameBoardPosition ) {
                result = YES;
            }
        }
        }
        ...
        if ( (tempCheckPositionDiceArray[0].gameBoardPosition == 0 &&
              tempCheckPositionDiceArray[1].gameBoardPosition == 5 &&
              tempCheckPositionDiceArray[2].gameBoardPosition == 10 &&
              tempCheckPositionDiceArray[3].gameBoardPosition == 15 )
            || (tempCheckPositionDiceArray[0].gameBoardPosition == 3 &&
                tempCheckPositionDiceArray[1].gameBoardPosition == 6 &&
                tempCheckPositionDiceArray[2].gameBoardPosition == 9 &&
                tempCheckPositionDiceArray[3].gameBoardPosition == 12)) {
                result = YES;
        }
        }
            break;
        ...
        case enumFirstPlayThreeDifRuleCondition3:
        {
            ...
            else if(tempCheckPositionDiceArray[0].gameBoardPosition/4 == tempCheckPositionDiceArray[1].gameBoardPosition/4 &&
                     tempCheckPositionDiceArray[0].gameBoardPosition/4 == tempCheckPositionDiceArray[2].gameBoardPosition/4 &&
                     tempCheckPositionDiceArray[0].gameBoardPosition/4 == tempCheckPositionDiceArray[3].gameBoardPosition/4) {
                result = YES;
            }
            else if( tempCheckPositionDiceArray[1].gameBoardPosition - tempCheckPositionDiceArray[0].gameBoardPosition == 1 && (tempCheckPositionDiceArray[0].gameBoardPosition != 3 || tempCheckPositionDiceArray[0].gameBoardPosition != 7 || tempCheckPositionDiceArray[0].gameBoardPosition != 11 || tempCheckPositionDiceArray[0].gameBoardPosition != 12)) {
                if ( tempCheckPositionDiceArray[0].gameBoardPosition + 4 == tempCheckPositionDiceArray[2].gameBoardPosition &&  tempCheckPositionDiceArray[1].gameBoardPosition + 4 == tempCheckPositionDiceArray[3].gameBoardPosition ) {
                    result = YES;
                }
            }
            else if( (tempCheckPositionDiceArray[0].gameBoardPosition == 0 &&
                       tempCheckPositionDiceArray[1].gameBoardPosition == 5 &&
                       tempCheckPositionDiceArray[2].gameBoardPosition == 10 &&
                       tempCheckPositionDiceArray[3].gameBoardPosition == 15 )
                     || (tempCheckPositionDiceArray[0].gameBoardPosition == 3 &&
            ...
            break;
        case enumFirstPlayFourDiceComboCondition:
            if ( tempCheckPositionDiceArray[0].gameBoardPosition == 10 ) {
                result = YES;
            }
            break;
        case enumFirstPlaySuperDiceCreateCondition:
        {
            for ( SuperDice *tempSuperDice in superDiceArray ) {
                tempCheckPositionDiceArray[0] = tempSuperDice;
            }
            if ( tempCheckPositionDiceArray[0].gameBoardPosition == 9 ) {
                lockRedButton = NO;
                [rollButtonMenu setIsTouchEnabled:YES];
                result = YES;
                [tempShowSprite setOpacity:255];
            }
            else{
                [tempShowSprite setOpacity:0];
                lockRedButton = YES;
...
-(void)createSuperDice:(int)createSuperDiceNumber{
	if ( createSuperDiceNumber == 0 || [superDiceArray count] + createSuperDiceNumber > 4) {
		return;
	}
    if (talkBubbleTag != enumTalkBubbleSuperDice) {
        if ( talkBubbleTag == enumTalkBubbleNPCTalk ) {
...
-(void)changeTalkContext:(NSSet *)touch withNumber:(int)tempNumber{
    for ( SuperDice *tempSuperDice in superDiceArray ) {
        if ( tempSuperDice.position.y <= -40) {
            return;
        }
    ...
    [talkLabel setEventTextWithSequence:tempNumber];
}
